...
class DrawerNotification extends React.Component {
  ...
  getNotification = (lastCreatedAt = null) => {
    ...
    fetchAPI({
      ...
      const { allProject } = this.props;
      const notifications = this.state.notifications.concat(res.data.data);
      notifications.forEach(notify => {
        const newNotify = allProject.find(project => notify.service.toLowerCase() === project.projectId.toLowerCase());
        if (newNotify) {
  ...
  render () {
    ...
    const operations = <MoreOption onClose={onClose} markAsRead={this._markAsReadAll} />;
    ...
    return (
      ...
      >
        ...
        <Tabs defaultActiveKey="1" onChange={callback} tabBarExtraContent={operations}>
          ...
          <TabPane className="tab-notification" tab={`${i18n.t('header:system')} (${numberOfNotificationText})`}
            key="1">
